-- DATABASE: playstore
-- relation is called "analytics"

-- columns are: id, app_name, category, rating, reviews, size, min_installs, price, content_rating, genres, last_updated, current_version, android_version
-- date format example: 2018-08-03
-- reviews is a count of how many reviews the app has
...
SELECT category, ROUND( AVG(rating)::NUMERIC, 2) AS avg_rating FROM analytics GROUP BY category ORDER BY avg_rating desc;
...
SELECT SUM(reviews) AS total_playstore_reviews FROM analytics;
...
-- min_installs:reviews proportion
SELECT app_name, min_installs, reviews, min_installs/reviews AS install_to_reviews_proportion FROM analytics WHERE min_installs>=100000;